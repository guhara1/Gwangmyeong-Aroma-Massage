...
규칙(자동 적용):
  ...
  - 번호 행정동(광명1동 등)·교통수단별 중복 경로는 생성하지 않는 구조
"""
import html
import os
import re
...
<header class="site-header">
  ...
  <div class="header-top">
    <div class="header-inner">
      ...
      <p class="header-tagline"><span class="tag-gem">◆</span> 광명시 전지역 방문 관리 <span class="tag-gem">◆</span> 24시간 상담</p>
...
<footer class="site-footer">
  <div class="container footer-grid">
    <div class="footer-col footer-about">
      ...
      <p class="footer-desc">광명시 전지역 방문 출장마사지·홈타이 안내 사이트입니다. 모든 서비스는 안내된 관리 범위와 위생·안전 기준 안에서만 제공됩니다.</p>
      <address class="footer-contact">
        ...
        <span class="footer-contact-row"><span class="footer-label">서비스 지역</span> 경기도 광명시 전지역</span>
      </address>
    </div>
    <nav class="footer-col" aria-label="대표 행정동 안내">
    ...
    <nav class="footer-col" aria-label="역세권 안내">
      <p class="footer-title">역세권</p>
      <ul>
    ...
    <nav class="footer-col" aria-label="이용 안내">
      ...
      <ul>
        ...
        <li><a href="/hometai/">홈타이 가이드</a></li>
...
def build() -> None:
    ...
    for page in PAGES:
        path = page["path"]  # "" 또는 "gwangmyeong/...-dong-chuljangmassage/" 형태
    ...
    sitemap_urls = [u for u, _, _ in sitemap_items]

    # sitemap.xml — 색인 우선도(priority)와 변경 시각(lastmod) 포함
    def _priority(loc):
        if loc.rstrip("/") == base:
            return "1.0"
        return "0.8"
    ...
    urls = "\n".join(
        ...
        f"<changefreq>weekly</changefreq><priority>{_priority(loc)}</priority></url>"
    ...
    # rss.xml — 네이버·구글 등 피드 수집기용 RSS 2.0 피드
    def _esc(s):
    ...
    with open(os.path.join(ROOT, "rss.xml"), "w", encoding="utf-8") as f:
        f.write(
            ...
            "  <channel>\n"
            f"    <title>{_esc(BRAND)} — 광명 출장마사지·홈타이 안내</title>\n"
            f"    <link>{base}/</link>\n"
            f'    <atom:link href="{base}/rss.xml" rel="self" type="application/rss+xml" />\n'
            "    <description>광명시 전지역 방문 출장마사지·홈타이 지역별 안내</description>\n"
    ...
    # robots.txt — 사이트맵·RSS 위치 명시 (네이버·구글·빙 크롤러 허용)
    ...
    # IndexNow 키 파일 — 루트에 {KEY}.txt 로 키 값을 게시한다.
    with open(os.path.join(ROOT, f"{INDEXNOW_KEY}.txt"), "w", encoding="utf-8") as f:
        f.write(INDEXNOW_KEY + "\n")

    # .nojekyll (정적 호스팅에서 _ 디렉터리 등 보존)
